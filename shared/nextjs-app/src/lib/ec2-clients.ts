...
export async function startInstance(input: StartInstanceInput): Promise<InstanceInfo> {
  ...
  const result = await ec2Client.send(new RunInstancesCommand({
    ...
    UserData: Buffer.from([
      ...
      `# MCP config sync (department gateway URLs from DynamoDB)`,
      `IMDS_TOKEN=$(curl -s -X PUT http://169.254.169.254/latest/api/token -H 'X-aws-ec2-metadata-token-ttl-seconds:60' 2>/dev/null)`,
      `INST_ID=$(curl -s -H "X-aws-ec2-metadata-token:$IMDS_TOKEN" http://169.254.169.254/latest/meta-data/instance-id 2>/dev/null)`,
      `DEPT_TAG=$(aws ec2 describe-tags --filters "Name=resource-id,Values=$INST_ID" "Name=key,Values=department" --query 'Tags[0].Value' --output text --region ${region} 2>/dev/null || echo "default")`,
      `COMMON_GW=$(aws dynamodb get-item --table-name cc-dept-mcp-config --key '{"PK":{"S":"COMMON"},"SK":{"S":"GATEWAY"}}' --query 'Item.gatewayUrl.S' --output text --region ${region} 2>/dev/null || echo "None")`,
      `DEPT_GW=$(aws dynamodb get-item --table-name cc-dept-mcp-config --key '{"PK":{"S":"DEPT#'"$DEPT_TAG"'"},"SK":{"S":"GATEWAY"}}' --query 'Item.gatewayUrl.S' --output text --region ${region} 2>/dev/null || echo "None")`,
      `sudo -u coder mkdir -p /home/coder/.claude`,
      `MCP=/home/coder/.claude/mcp_servers.json`,
      `echo '{' > $MCP`,
      `echo '  "awslabs-core-mcp-server":{"command":"uvx","args":["awslabs.core-mcp-server@latest"],"env":{"AWS_REGION":"${region}"}},' >> $MCP`,
      `echo '  "bedrock-agentcore-mcp-server":{"command":"uvx","args":["bedrock-agentcore-mcp-server@latest"],"env":{"AWS_REGION":"${region}"}}' >> $MCP`,
      `[ "$COMMON_GW" != "None" ] && [ -n "$COMMON_GW" ] && echo '  ,"cc-common-gateway":{"url":"'$COMMON_GW'"}' >> $MCP`,
      `[ "$DEPT_GW" != "None" ] && [ -n "$DEPT_GW" ] && echo '  ,"cc-dept-gateway":{"url":"'$DEPT_GW'"}' >> $MCP`,
      `echo '}' >> $MCP`,
