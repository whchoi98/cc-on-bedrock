...
<Screenshot 
  src="/cc-on-bedrock/img/home.png" 
  alt="Dashboard Home" 
...
## 2. 내 환경 (My Environment)

사용자가 자신의 개발환경을 직접 관리하는 셀프서비스 포털입니다. 3개 탭으로 구성됩니다.

- **환경 정보**: SSE 실시간 프로비저닝 (6단계), 컨테이너 상태, VSCode URL, CPU/Memory 메트릭
- **스토리지**: 디스크 사용량 게이지, EBS 확장 신청, Keep-Alive
- **설정**: Code-server 비밀번호 관리 (Cognito 동기화), 계정 정보

:::tip 자세한 내용
[내 환경 (My Environment) 전체 가이드](/docs/user-portal)에서 SSE 프로비저닝 파이프라인, 비밀번호 동기화 구조, API 레퍼런스를 확인하세요.
:::

## 3. AI 어시스턴트 (AI Assistant)
Amazon Bedrock의 Converse API를 활용하여 실시간 스트리밍 답변을 제공하는 지능형 비서입니다.

<Screenshot 
  src="/cc-on-bedrock/img/AI_Assistant.png" 
  alt="AI Assistant" 
...
<Screenshot 
  src="/cc-on-bedrock/img/user.png" 
  alt="User Management" 
