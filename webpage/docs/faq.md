...
```
첫 시작: 새 gp3 볼륨 생성 (20GB, 3000 IOPS) → /home/coder에 마운트
정지 시: SIGTERM → S3 풀백업 → EBS 스냅샷 생성 → 볼륨 삭제
재시작: 스냅샷에서 새 볼륨 생성 → /home/coder에 마운트 (데이터 복원)
```
...
사용자 일일 예산:
  80%: SNS 경고 알림
  100%: IAM Deny Policy 부착 → Bedrock API 호출 차단 + Cognito 플래그
  다음 날: 자동 Deny Policy 해제

부서 월간 예산:
  80%: 부서 관리자에게 SNS 경고
  100%: 부서 전체 사용자 IAM Deny Policy 부착
  예산 이하로 내려가면: 자동 해제
```
