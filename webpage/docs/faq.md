...
:::info EBS는 AZ에 종속됩니다
EC2-per-user 아키텍처에서는 Stop/Start로 EBS가 자동 보존되므로 **Snapshot이 불필요**합니다. AZ 장애 시에만 Admin이 수동 Snapshot → 다른 AZ 복구합니다.
:::

### EC2 Stop/Start로 모든 데이터가 보존되나요?

네. EC2 Stop 시 EBS root volume은 자동 보존됩니다:
...
:::tip ECS 대비 장점
ECS에서는 EBS snapshot/restore, /usr/local symlink hack, S3 backup 등 복잡한 우회가 필요했지만, EC2에서는 **Stop = 보존, Start = 복원**으로 단순합니다.
:::
...
| 상태 | 컴퓨트 비용 | 스토리지 비용 |
|------|-----------|-------------|
| **Running** (t4g.large) | $0.0672/hr | $0.08/GB/월 |
| **Stopped** | **$0** | $0.08/GB/월 (30GB = $2.4/월) |

100명 동시 사용, 400명 등록 기준: ~$6,200/월 (ECS 대비 ~14% 절감)
